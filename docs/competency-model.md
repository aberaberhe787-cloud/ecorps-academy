...
│  Mission, Exam) │       │  Demonstration, Pass)│       │  Practicing, etc.)│
└─────────────────┘       └──────────────────────┘       └───────────────────┘
```
...
The system defines seven core competencies derived directly from the curriculum and engineering sandbox:
...
| `adversarial-defense` | **System Alignment & Adversarial Robustness** | Hardening system prompts against injection vectors, jailbreaks, data leaks, and instruction overrides. | Prompt Injection Defense, Dual-LLM Sandboxing, Secret Containment, Adversarial Fuzzing |
| `evaluation-orchestration` | **Evaluation, Tool Calling & Agentic Orchestration** | ReAct execution loops, function-calling schema binding, automated benchmark grading, and metric-based iteration. | ReAct Loops, Tool Calling, Automated Rubric Evaluation, Pedagogical Graders |
...
```
                  ...
                  │ Solved Missions & Adversarial CTF Def. │
                  └───────────────────┬────────────────────┘
                                      │
                  ┌───────────────────▼────────────────────┐
...
3. **Demonstration (`demonstration`)**: Verified problem-solving. The learner satisfied deterministic constraints in multi-step missions or defeated adversarial attacks in the CTF arena.
...
* **Trigger**: Learner completes at least $1$ hands-on demonstration challenge ($\ge 1\text{ demonstration}$) in a mission or CTF level, OR demonstrates deep practice ($\ge 3\text{ practice units}$).
* **Pedagogical Meaning**: The learner has proven they can generate functional prompts that satisfy strict external constraints (e.g., regex checks, zero-preamble JSON parsing, or prompt injection defense).
...
* **Trigger**: Learner passes the authoritative **Capstone Assessment** for the domain ($\ge 1\text{ assessment}$) OR demonstrates repeated mastery across multiple challenges ($\ge 2\text{ demonstrations}$ with $\ge 1\text{ practice}$ and $\ge 4\text{ total evidence units}$).
* **Pedagogical Meaning**: The highest standard of achievement. Capability has been proven under formal assessment conditions or through comprehensive, multi-angle practical problem solving.
...
The function `deriveCompetencyStates(progress, ctfSolved)` is a pure TypeScript function. Given the same set of completed lesson IDs, mission IDs, and CTF challenge states, it will **always** compute the exact same competency level and evidence trail. No random sampling, no temperature variations, and no model drift.

### 2. Transparent Audit Trail
Every competency level is directly traceable to discrete evidence items:
```json
{
  "id": "ev_mission_mission-2",
  "type": "mission_completed",
  "sourceId": "mission-2",
  "title": "Customer Review JSON Serializer",
  "timestamp": 1726740000000,
  "weight": "demonstration",
  "summary": "Passed mission verification challenge: Customer Review JSON Serializer"
}
```
Learners can inspect exactly which missions, lessons, or exams contributed to their status.

### 3. Separation of Gamification vs. Competency
XP points and streak multipliers incentivize daily consistency, but **XP alone never unlocks competency status**. A user with 50,000 XP who has only clicked through reading material will remain at `Introduced` or `Practicing` until they solve practical challenges.

### 4. Objective Sandbox & CTF Validation
Challenges rely on deterministic validators:
* **JSON/Schema Invariant Checks**: Verifies valid syntax, required keys, and zero preamble.
...
All evidence derivation and local cache storage respect strict user boundaries (`promptlab_user_progress_${uid}` and `ecorp_ctf_solved_${uid}`). Unauthenticated guest state is strictly sandboxed to prevent privilege escalation or false credit across accounts.

---

## 6. Developer Integration & Extension Points

To map a new lesson, mission, or pattern to a competency:
