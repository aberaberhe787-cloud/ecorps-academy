...
export const LoginPage: React.FC = () => {
  ...
  return (
    <main
      id="login-page-root"
      className="login-page min-h-dvh w-full max-w-full min-w-0 mx-auto bg-[#050a19] px-3 sm:px-4 min-[960px]:px-6 lg:px-8 py-3 sm:py-4 min-[960px]:py-4 lg:py-6 text-slate-100 flex flex-col justify-start min-[960px]:justify-center items-center gap-3 sm:gap-4 min-[960px]:gap-4 lg:gap-5 overflow-y-auto pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))]"
    >
      {/* Mobile Top Brand Bar - Compact branding on small phone screens (< 960px) */}
      <header className="min-[960px]:hidden flex items-center justify-between mb-1.5 sm:mb-3 px-1 w-full min-w-0 max-w-md mx-auto shrink-0">
      ...
      {/* Main Responsive Container: Fluid sizing across device widths, 1-column stack below 960px, 2-column grid at 960px+ */}
      <motion.div
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
        className="w-full max-w-full min-w-0 mx-auto max-w-6xl grid grid-cols-1 min-[960px]:grid-cols-2 rounded-2xl lg:rounded-3xl border border-slate-800/80 bg-[#080e20]/95 shadow-2xl shadow-black/40 overflow-hidden my-auto min-h-0"
      >
        
        {/* SIGN-IN FORM: Prominent, fluid width, stacked below marketing on mobile/tablet, side-by-side on 960px+ */}
        <section
          id="auth-form-section"
          className="w-full min-w-0 max-w-full order-1 min-[960px]:order-2 min-[960px]:col-start-2 min-[960px]:row-start-1 relative flex flex-col items-center justify-center p-[clamp(0.75rem,2vw,1.75rem)] overflow-hidden"
        >
          <motion.div
            initial={{ opacity: 0, y: 10, scale: 0.99 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            transition={{ duration: 0.4, delay: 0.08, ease: [0.22, 1, 0.36, 1] }}
            className="w-full min-w-0 max-w-md mx-auto rounded-xl lg:rounded-2xl p-1 sm:p-2.5 lg:p-6 lg:border lg:border-indigo-500/40 lg:bg-[#0c142a]/95 lg:shadow-[0_0_35px_rgba(37,99,235,.08)]"
          >
            ...
            <div className="w-full min-w-0 mt-3.5 sm:mt-4 grid grid-cols-2 gap-2.5 sm:gap-3">
              <button
                type="button"
            ...
            <form onSubmit={handleEmailAuth} className="w-full min-w-0 space-y-3 sm:space-y-3.5" noValidate>
              {isSignUp && (
        ...
        <section
          ...
          className="flex w-full min-w-0 max-w-full order-2 min-[960px]:order-1 min-[960px]:col-start-1 min-[960px]:row-start-1 relative flex-col justify-between overflow-hidden p-[clamp(0.75rem,2vw,2rem)] border-t min-[960px]:border-t-0 min-[960px]:border-r border-slate-800/60 gap-[clamp(0.5rem,1.5vw,1rem)]"
        >
          ...
          <div className="hidden min-[960px]:flex w-full min-w-0 relative z-10 items-center gap-[clamp(0.5rem,1vw,0.75rem)]">
          ...
          <div className="relative z-10 w-full min-w-0 max-w-full flex flex-col gap-[clamp(0.25rem,0.8vw,0.625rem)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[clamp(0.35rem,1vw,0.65rem)] w-full min-w-0">
              {[
                {
                  icon: BookOpen,
                ...
                <div key={title} className="tablet-feature-card flex items-center gap-2.5 rounded-xl bg-slate-900/60 p-2 sm:p-2.5 border border-slate-800/70 min-w-0 transition-colors hover:border-slate-700/80">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs sm:text-sm md:text-sm font-semibold text-slate-200 leading-snug">{title}</p>
          ...
          <div className="hidden min-[960px]:block w-full min-w-0 relative z-10 pt-2 lg:pt-4 border-t border-slate-800/60 text-xs md:text-xs text-slate-500">
            <span>Trusted by builders learning modern AI orchestration</span>
          </div>
        </section>
      </motion.div>

      {/* Footer Features Bar: Fluid 2x2 grid on tablet (768px-959px), single stack on mobile, 4-item horizontal on landscape/desktop */}
      <footer
        id="login-footer-feature-statistics"
        className="w-full max-w-full min-w-0 mx-auto max-w-6xl grid grid-cols-1 sm:grid-cols-2 min-[960px]:flex min-[960px]:flex-row min-[960px]:items-center min-[960px]:justify-between gap-[clamp(0.4rem,1.2vw,0.75rem)] min-[960px]:gap-3 lg:gap-6 py-2 sm:py-3 lg:py-5 text-xs text-center px-0 shrink-0"
      >
        {FOOTER_FEATURES.map((feat, index) => {
          ...
          return (
            <div
              key={feat.text}
              className="w-full min-[960px]:w-auto relative group overflow-hidden rounded-xl p-[1px] transition-all duration-300 hover:scale-[1.02] shadow-sm shadow-black/30 shrink-0"
            >
        ...
        {isForgotPasswordOpen && (
          ...
          >
            <motion.div
              ...
              className="w-full min-w-0 max-w-md rounded-2xl border border-slate-800/90 bg-[#080e20] p-5 sm:p-7 shadow-2xl shadow-blue-950/40 text-slate-100 relative my-auto min-h-0"
            >
              {/* Close Button */}
              <button
                type="button"
