...
export const HomeView: React.FC = () => {
  ...
  return (
    <div className="app-view space-y-16 py-8">
      {/* Learner control center */}
      <section className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-2xl border border-blue-900/60 bg-slate-900/90 p-5 shadow-xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-blue-400">Learning Hub</p>
              <h2 className="mt-1 text-2xl font-bold text-white">Your learning command center</h2>
              <p className="mt-1 text-sm text-slate-400">Resume a lesson, practice a skill, or review your progress.</p>
            </div>
            <button type="button" onClick={() => setActiveTab("foundations")} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
            <button type="button" onClick={() => nextFoundation ? setActiveTab("foundations") : setActiveTab("curriculum")} className="rounded-xl border border-blue-800/70 bg-blue-950/40 p-4 text-left hover:border-blue-500">
              ...
              <h3 className="mt-2 text-lg font-bold text-white">{nextFoundation?.title || nextLesson?.title || "All lessons complete"}</h3>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4"><p className="text-xs text-slate-400">Overall progress</p><p className="mt-2 text-2xl font-bold text-white">{totalCompletedLessons}/{totalTrackedLessons}</p><div className="mt-3 h-2 rounded-full bg-slate-800"><div className="h-full rounded-full bg-blue-500" style={{ width: `${totalTrackedLessons ? (totalCompletedLessons / totalTrackedLessons) * 100 : 0}%` }} /></div></div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4"><p className="text-xs text-slate-400">Weekly goal</p><p className="mt-2 text-2xl font-bold text-emerald-400">{weeklyGoal}/5</p><p className="mt-2 text-xs text-slate-500">Lessons completed</p></div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4"><p className="text-xs text-slate-400">Streak and XP</p><p className="mt-2 text-lg font-bold text-amber-300">{userProgress.streakDays} days</p><p className="text-xs text-slate-500">{userProgress.xp} XP earned</p></div>
          </div>
      ...
      <section className="relative overflow-hidden px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-screen-xl text-center">
          ...
          {/* Subtitle */}
          <p className="mx-auto mt-4 max-w-2xl text-base text-slate-300 leading-relaxed sm:text-lg">
            {t.home.heroSubtitle}
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/25 transition-all hover:brightness-110 active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/90 px-6 py-3 text-sm font-semibold text-slate-200 transition-all hover:bg-slate-800 hover:text-white"
            >
          ...
          <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
              <span className="text-2xl font-extrabold text-blue-400 font-mono">10</span>
              <p className="text-xs text-slate-400 mt-0.5">{t.home.statsLessons}</p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
              <span className="text-2xl font-extrabold text-emerald-400 font-mono">5</span>
              <p className="text-xs text-slate-400 mt-0.5">{t.home.statsMissions}</p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
              <span className="text-2xl font-extrabold text-purple-400 font-mono">6+</span>
              <p className="text-xs text-slate-400 mt-0.5">{t.home.statsPatterns}</p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
              <span className="text-2xl font-extrabold text-amber-400 font-mono">100%</span>
      ...
      <section className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8 space-y-6">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold tracking-tight text-white">{t.home.keyPillarsTitle}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 cursor-pointer hover:bg-slate-800 transition-colors" onClick={() => setActiveTab("curriculum")}>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 cursor-pointer hover:bg-slate-800 transition-colors" onClick={() => setActiveTab("playground")}>
      ...
      <section className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-slate-800 bg-gradient-to-b from-slate-900/90 to-slate-950 p-8 sm:p-12">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-bold text-white shadow-md hover:bg-blue-500 transition-all"
                >
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5 font-mono text-xs text-slate-300 space-y-2.5">
              ...
              <div className="space-y-1.5 text-[11px]">
                <div className="flex items-center gap-1.5 text-emerald-300">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" /> Persona & Domain Expertise calibrated
                </div>
                <div className="flex items-center gap-1.5 text-emerald-300">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" /> XML Delimiters protect input integrity
                </div>
                <div className="flex items-center gap-1.5 text-emerald-300">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" /> Strict raw JSON schema enforced
                </div>
                <div className="flex items-center gap-1.5 text-emerald-300">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" /> Negative constraints prevent preamble
                </div>
              </div>
            </div>
          </div>
